...
(In a "real" scenario you'd load this data directly from DB; Does not matter for our performance tests though)
...
Redis is used for caching (does not really make any difference.  
It seems in terms of the API endpoint it's only used to cache the rate-limit data)
...
* "suggest" lookup: **~190x** faster (**7,22** requests/s vs. **1353,17** r/s)
* "info" lookup: **~65x** faster (**117,53** r/s vs. **7580,58** r/s)
* "search" lookup: **~33x** faster (**5,29** r/s vs. **178,51**) 
